      ...
      <div className="w-full h-full shadow-lg p-3">
        ...
        <div className="grid lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-6 mt-3">
          {books.map((book) => (
            ...
            >
              {/* Image area (fixed height so cards consistent) */}
              <div className="flex items-center justify-center h-56 overflow-hidden rounded-md">
                {/* pastikan book.frontImg menyesuaikan ukuran (img with object-cover) */}
                {book.frontImg}
              </div>

              {/* Title area (batasi tinggi agar seragam) */}
              ...
              {/* Tombol: dorong ke bawah dengan mt-auto agar sejajar */}
              <div className="mt-auto pt-4 flex justify-center">
                <Link href={`${thisLink}${book.id.toString()}`} className="w-full sm:w-auto">
                  {/* Jika Ebook_button_detail menerima className, bisa pass props.
                      Jika tidak, pastikan internal komponennya sudah style responsif. */}
                  <Ebook_button_detail />
                </Link>
              </div>
            </div>
