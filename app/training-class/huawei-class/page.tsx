...
import { huawei_data } from "./kelas_data_huawei";

// Props harus awaitable
export default async function TrainingHuawei({
  params,
}: {
  params: Promise<{ kelas_huawei_Id: string }>;
}) {
  const { kelas_huawei_Id } = await params; // wajib await
  const kelas = huawei_data.find((k) => k.id.toString() === kelas_huawei_Id);
