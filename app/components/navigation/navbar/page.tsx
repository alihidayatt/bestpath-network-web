...
const menuItems = [
  ...
  },
  {
    id: 2,
    title: "PROJECTS",
    children: [{ id: 1, title: "TRAINING EVENT", route: "/training" }],
  },
  {
    id: 3,
    title: "PARTNERS",
...
export default function Navbar() {
  ...
  return (
    <div className="flex justify-between items-center w-full h-20 px-4 text-bluegreen bg-white fixed top-0 shadow-custom1 z-50">
      <div className="container mx-auto flex justify-between items-center w-full">
        ...
        <ul className="hidden md:flex gap-x-4">
          {menuItems.map((menu) =>
            menu.children ? (
              <Menu key={menu.id} as="div" className="relative inline-block text-left">
                <div>
              ...
              <Link
                key={menu.id}
                href={menu.route || "#"}
                className="inline-flex justify-center gap-x-3 rounded-md bg-white px-3 py-2 text-lg font-semibold text-bluegreen hover:bg-gray-50"
              >
                {menu.title}
              </Link>
            )
        ...
        {/* MOBILE MENU */}
        {nav && (
          <ul className="flex flex-col justify-center items-center absolute top-0 left-0 w-full h-screen bg-white text-bluegreen font-medium z-40">
            {menuItems.map((menu) =>
              menu.children ? (
                <Menu key={menu.id} as="div" className="relative inline-block text-center">
                  <div className="my-3">
                    <Menu.Button className="inline-flex justify-center gap-x-3 rounded-md bg-white px-3 py-2 text-lg font-semibold text-bluegreen hover:bg-gray-50">
                      {menu.title}
                      <ChevronDownIcon
                ...
                <Link
                  key={menu.id}
                  href={menu.route || "#"}
                  className="block my-3 text-lg font-semibold text-bluegreen hover:bg-gray-50 px-3 py-2"
                  onClick={() => setNav(false)}
                >
                  {menu.title}
                </Link>
